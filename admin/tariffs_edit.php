...
$tariff_id = null;
$tariff = []; // Array untuk menyimpan data tarif

// 2. Ambil ID Tarif dari URL (GET Request)
if (isset($_GET['id'])) {
    $tariff_id = $_GET['id'];

    // 3. Ambil data lama dari database
    $stmt_select = $koneksi->prepare("SELECT * FROM power_tariffs WHERE id = ?");
    $stmt_select->bind_param("i", $tariff_id); // 'i' untuk integer
    $stmt_select->execute();
...
// Jika data $tariff kosong (karena error atau ID tidak ada), isi dengan string kosong
if (empty($tariff)) {
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    ...
                    <?php if (!empty($tariff)): // Hanya tampilkan form jika data ditemukan ?>
                        <div class="card mb-4">
