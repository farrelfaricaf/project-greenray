...
$user_id = null;
$user = []; // Array untuk menyimpan data user yang akan diedit

// 2. Ambil ID User dari URL (GET Request)
if (isset($_GET['id'])) {
    $user_id = $_GET['id'];

    // 3. Ambil data user yang ada dari database (JANGAN AMBIL PASSWORD)
    $stmt_select = $koneksi->prepare("SELECT id, first_name, last_name, email FROM users WHERE id = ?");
    $stmt_select->bind_param("i", $user_id); // 'i' untuk integer
    $stmt_select->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $email = $_POST['email'];
    // Kita tidak meng-update password di sini. Itu ditangani oleh user_reset_password.php

    // 5. Buat query UPDATE
    $stmt_update = $koneksi->prepare("UPDATE users SET first_name = ?, last_name = ?, email = ? WHERE id = ?");

    // 'sssi' = string, string, string, integer (untuk ID)
    ...
    if ($stmt_update->execute()) {
        ...
        // Cek jika errornya karena email duplikat
        if ($koneksi->errno == 1062) { // 1062 = Error duplikat entry
...
// Jika data $user kosong (karena error atau ID tidak ada), isi dengan string kosong
if (empty($user)) {
